...
import {
  ...
  setSelectedSector,
  updateQuestion,
} from "../features/questionManageSlice";
...
const ManageQuestion = () => {
  ...
  const {
    ...
    selectedJobRole,
    questions,
  } = useSelector((state) => state.questionManage);

  const showButtons =
    selectedSector && selectedJobRole && selectedPaperSet && !formVisible;

  const isTranslated =
    questions
      .map(
        (question) =>
          question.translations && question.translations[selectedLang]
      )
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      if (isViva) {
        ...
        if (
          firstCorrectOption &&
          !formattedOptions.some((opt) => opt.isCorrect)
        ) {
          formattedOptions.push({
  ...
  const downloadQuestions = () => {
    ...
    if (languages.size === 0) {
      ...
      const formattedData = data.map((question) => {
        ...
        const correctOptionAlphabet = correctOption
    ...
    languages.forEach((lang) => {
      ...
      const formattedData = data.map((question) => {
        const options = question.options.map(
          (option) => option.translations[lang] || option.option
        );
        const correctOption = question.options.find(
          (option) => option.isCorrect
        );
  ...
  const handleUpload = async () => {
    ...
    reader.onload = async (e) => {
      ...
      const transformedData = workbook.SheetNames.flatMap((sheetName) => {
        ...
        return jsonData
          .map((row, index) => {
            ...
            const validOption2 =
              option2 !== undefined ? `${option2}`.trim() : "";
            const validOption3 =
              option3 !== undefined ? `${option3}`.trim() : "";
            const validOption4 =
              option4 !== undefined ? `${option4}`.trim() : "";
            const validCorrectOption = correctOption
              ? `${correctOption}`.trim().toLowerCase()
              : "";
            const validMarks = marks !== undefined ? Number(marks) : null;
            const validDifficultyLevel = difficultyLevel
              ? `${difficultyLevel}`.trim().toLowerCase()
              : "";
            let validQuestionType = questionType
            ...
            if (
              ...
              validationErrors.push(
                `Row ${
                  index + 1
                } in sheet ${sheetName}: Missing or invalid values.`
              );
              return null;
            }
            ...
            if (options.length > 0 && !options.some((opt) => opt.isCorrect)) {
              validationErrors.push(
                `Row ${
                  index + 1
                } in sheet ${sheetName}: There should be at least one correct answer.`
              );
              return null;
            }
            ...
            if (!nos) {
              validationErrors.push(
                `Row ${
                  index + 1
                } in sheet ${sheetName}: No matching NOS found for sheet name.`
              );
              return null;
            }
      ...
      try {
        await axios.post(
          `${BASE_URL}company/questions/bulk-insert`,
  ...
  const handleUpdate = async (e) => {
    ...
    const resultAction = await dispatch(
      updateQuestion({ _id, updatedQuestion })
    );
  ...
  const filteredQuestions = selectedPaperSet
    ? questions.filter(
        (question) => question.questionBank._id === selectedPaperSet._id
      )
    : [];
  ...
  const renderHTMLContent = (rowData) => {
    const title = rowData.translations
      ? !Object.keys(rowData.translations).includes(selectedLang)
        ? rowData.title
        : rowData.translations[selectedLang]
      : rowData.title;
  ...
  const handleTranslate = async () => {
    try {
      const response = await axios.patch(
        `${BASE_URL}company/question-banks/${selectedPaperSet._id}/translate`,
    ...
    >
        ...
        {!formVisible && (
          <div className="grid grid-cols-1 p-2 md:grid-cols-3 gap-4 bg-gray-100 rounded-lg py-2 pb-6">
            ...
            <div className="flex flex-col flex-1 space-y-4 my-2">
              <label htmlFor="questionset" className="font-semibold">
                Select Question set{" "}
              </label>
              <select
                id="questionset"
        ...
        {showButtons && (
          <div className="flex flex-col md:flex-row items-center mb-4 space-y-4 md:space-y-0 md:space-x-4">
            ...
            <select
              ...
              className="p-1.5 px-5 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 transition"
            >
              <option value="en" className="pr-2">
                English
              </option>
              <option value="hi" className="pr-2">
                Hindi
              </option>
              <option value="mr" className="pr-2">
                Marathi
              </option>
              <option value="ta" className="pr-2">
                Tamil
              </option>
              <option value="te" className="pr-2">
                Telugu
              </option>
              <option value="kn" className="pr-2">
                Kannada
              </option>
              <option value="gu" className="pr-2">
                Gujarati
              </option>
              <option value="bn" className="pr-2">
                Bengali
              </option>
              <option value="pa" className="pr-2">
                Punjabi
              </option>
              <option value="ml" className="pr-2">
                Malayalam
              </option>
              <option value="ur" className="pr-2">
                Urdu
              </option>
              <option value="or" className="pr-2">
                Odia
              </option>
              <option value="as" className="pr-2">
                Assamese
              </option>
            </select>
            ...
            {isBulkUploadVisible && (
              <>
                <div
                  {...getRootProps()}
                  className={`w-full md:w-2/3 lg:w-1/3 border-2 border-dashed rounded-lg p-6 mt-2 transition-all ease-in-out duration-300 ${
                    isDragActive
        ...
        <form onSubmit={handleSubmit} className="space-y-4 my-2 ">
          <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="flex flex-col flex-1">
              <label
                htmlFor="sector"
                className="mb-1 font-semibold text-lg ml-1"
              >
                Select Sector
              </label>
              <select
            ...
            <div className="flex flex-col flex-1 !ml-0">
              <label htmlFor="nos" className="mb-1 font-semibold text-lg ml-1">
                Select NOS Code
              </label>
              <select
                id="NOS"
            ...
            <div className="flex flex-col flex-1">
              <label htmlFor="pc" className="mb-1 font-semibold text-lg ml-1">
                Select PC
              </label>
              <select
                id="pc"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4 w-[365%]">
                {selectedPaperSet?.type === "viva" ? (
                  ...
                  ["A", "B", "C", "D"].map((label, index) => (
                    <div
                      key={index}
                      className="flex items-center space-x-4 w-full md:w-3/4 lg:w-2/3 relative"
                    >
                      <input
                        type="radio"
          ...
          {!selectedPaperSet && (
            <div className="text-red-500 text-md mt-2">
              Please select a Sector && Job Role and other information before
              entering Question detail.
            </div>
          )}
      ...
      {viewData && (
        ...
        >
          <div>
            <p>
              <strong>Question ID:</strong> {viewData._id}
            </p>
            <p>
              <strong>Difficulty Level:</strong> {viewData.difficultyLevel}
            </p>
            <p>
            ...
            <p>
              <strong>Total Marks:</strong> {viewData.marks}
            </p>
            <p>
              <strong>Job Role:</strong> {viewData.jobRole?.name}
            </p>
            <p>
              <strong>NOS:</strong> {viewData.nos?.nosName}
            </p>
          </div>
        </Dialog>
      )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
            <div className="flex flex-col flex-1">
              ...
              <select
                ...
                className="px-3 py-3 border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 transition"
              >
                <option>Select</option>
                <option value="easy">Easy</option>
                <option value="medium">Medium</option>
