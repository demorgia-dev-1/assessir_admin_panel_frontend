...
const ManageQuestionSet = () => {
    ...
    const { sectors, jobRoles, selectedSector, selectedJobRole, questionSets } = useSelector(state => state.questionSet);
    ...
    const handleEditFormSubmit = async (e) => {
        ...
        const resultAction = await dispatch(updateQuestionSet({ _id, updatedQuestionSet }));
    ...
    const filteredQuestionSet = selectedJobRole ? questionSets.filter(questionSet => questionSet.jobRole?._id === selectedJobRole._id) : [];
    ...
    return (
        ...
        mx-auto mt-5 p-0 sm:p-2  py-8 bg-white border-2 border-white/10 backdrop-blur-[20px] items-center overflow-hidden duration-200 ease-in-out text-black rounded-lg shadow-lg flex-grow-0 ">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div className="flex flex-col flex-1">
                        ...
                        >
                            ...
                            {sectors && sectors.length > 0 ? (
                                sectors.map((c) => (
                                    <option key={c._id} value={c._id}>{`${c.name?.toUpperCase()} (${c.sector_short_name?.toUpperCase()})`}</option>
                                ))
                            ) : (
                    ...
                    <div className="flex flex-col">
                        <label htmlFor="paperset" className="font-semibold p-2">Question Set Type</label>
                        <select id="paperset"
                            value={paperSetType}
                            onChange={(e) => setPaperSetType(e.target.value)} className="px-3 py-2 text-sm border-2 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 transition">
                            <option value="">Select</option>
                ...
                {paperSetType && (
                    <div className="flex space-x-4">
                        ...
                        <button type="submit" className="w-32 py-2 text-white bg-gradient-to-r from-purple-500 via-purple-600 to-purple-700 hover:bg-gradient-to-br shadow-md shadow-purple-500/50 dark:shadow-lg dark:shadow-purple-800/80 font-medium rounded-lg text-sm px-5 text-center transition-all duration-300 ease-in-out transform hover:-translate-y-1 hover:scale-110 flex items-center justify-center">
                            <span className='pr-2'><IoMdAdd className='w-4 h-5' /></span>
                            <span>Add</span>
                        </button>
                        <button type="button" onClick={handleClear} className="w-32 py-2 text-white bg-gray-600 hover:bg-gray-700 focus:ring-blue-300 shadow-md shadow-gray-500/50 dark:focus:ring-gray-800 font-medium rounded-lg text-sm px-5 text-center transition-all duration-300 ease-in-out transform hover:-translate-y-1 hover:scale-110 flex items-center justify-center">
                            <span className='pr-2'><VscClearAll className='w-4 h-5' /></span>
                            <span>Clear</span>
                        </button>
            ...
            {viewData && (
                <Dialog header="Question Set Details" visible={isViewFormVisible} style={{ width: '50vw' }} onHide={() => setIsViewFormVisible(false)}>
                    <div>
                        <p><strong>Question Set ID:</strong> {viewData._id}</p>
                        <p><strong>Question Set Name:</strong> {viewData.name}</p>
                        <p><strong>Question Set Type:</strong> {viewData.type}</p>
            ...
            <Dialog header="Edit Question Set" visible={isEditFormVisible} style={{ width: '50vw' }} onHide={() => setIsEditFormVisible(false)}>
                <form onSubmit={handleEditFormSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        <div className="flex flex-col">
                            <label htmlFor="name" className="mb-1 font-semibold text-lg ml-1">Question Set Name</label>
                            <input
                                type="text"
                                id="name"
                        ...
                        <div className="flex flex-col">
                            <label htmlFor="type" className="mb-1 font-semibold text-lg ml-1">Question Set Type</label>
                            <select id="type"
            ...
            <div className="mt-8 overflow-x-auto sm:w-full md:w-[100%] xl:w-full">
                ...
                <div className="max-w-[100%] p-1  md:w-full sm:max-w-full lg:max-w-[100%]">
                    ...
                    >
                        ...
                        <Column field="name" header="Question Set" className="py-2 px-4 border-b" />
