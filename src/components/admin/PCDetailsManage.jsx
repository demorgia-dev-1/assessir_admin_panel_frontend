...
const ManagePCDetails = () => {
    ...
    const { sectors, jobRoles, noses, selectedSector, selectedJobRole, selectedNOS, pcs } = useSelector(state => state.pc);

    const showButtons = selectedSector && selectedJobRole && !formVisible;

    const handleSubmit = (e) => {
        e.preventDefault();
        if (selectedSector && selectedJobRole && selectedNOS && pcName && totalMarks && totalMarksInTheory && totalMarksInPractical && totalMarksInViva && pcCode) {
            dispatch(createPC({
    ...
    const handleDownload = () => {
        const sampleData = [
            { "PC Name": "Patient Care", "PC Code": "HSC001", "Total Marks": 100, "Theory Marks": 50, "Practical Marks": 30, "Viva Marks": 20 },
        ];
    ...
    const handleUpload = async () => {
      ...
      allowEscapeKey: false, 
            ...
            try {
                const response = await axios.post(`${BASE_URL}company/pc/bulk-insert`, { pcs: transformedData }, {
                    headers: {
    ...
    const handleEditFormSubmit = async (e) => {
        ...
        try {
            const resultAction = await dispatch(updatePC({ _id: _id.toString(), updatedPC }));
    ...
    const filteredPCs = selectedJobRole ? pcs.filter(pc => pc.jobRole?._id === selectedJobRole._id) : [];
        ...
        mx-auto mt-14 sm:mt-5 p-0 sm:p-2  py-8 bg-white border-2 border-white/10 backdrop-blur-[20px] items-center overflow-hidden duration-200 ease-in-out text-black rounded-lg shadow-lg flex-grow-0 ">
            ...
            <div className="flex flex-col  space-y-4  pl- my-5">
                <div className="grid  grid-cols-1 md:grid-cols-3 gap-4 bg-gray-100 rounded-lg py-2 pb-6 px-2">
                    ...
                    <div className="flex flex-col">
                        <label htmlFor="jobrole" className="font-semibold  p-2">Select Job Role</label>
                        <select
                            id="jobrole"
            ...
            {formVisible && (<form onSubmit={handleSubmit} className="flex flex-col  space-y-4 my-5">
                <div className=" grid grid-cols-1 md:grid-cols-3 p-2 gap-4 rounded-lg py-2 pb-6">
                    <div className="flex flex-col flex-1 ">
                        <label htmlFor="nos" className="font-semibold ">Select NOS Code</label>
                        <select
                    ...
                    <div className="flex flex-col flex-1 ">
                        <label htmlFor="pcname" className="font-semibold">PC Name</label>
                        <input
                            type="text"
                            id="pcname"
                            value={pcName}
                    ...
                    <div className="flex flex-col flex-1">
                        <label htmlFor="pccode" className="font-semibold ">PC Code</label>
                        <input
                            type="text"
                            id="code"
                            value={pcCode}
                    ...
                    <div className="flex flex-col flex-1 ">
                        <label htmlFor="totalmarksintheory" className="font-semibold">Total Marks In Theory</label>
                        <input
                            type="number"
                    ...
                    <div className="flex flex-col flex-1 ">
                        <label htmlFor="totalmarksinpractical" className="font-semibold">Total Marks In Practical</label>
                        <input
                            type="number"
                    ...
                    <div className="flex flex-col flex-1  !ml-0">
                        <label htmlFor="totalmarksinviva" className="font-semibold ">Total Marks In Viva</label>
                        <input
                            type="number"
                            id="totalmarksinviva"
            ...
            {viewData && (
                <Dialog header="PC Details" visible={isViewFormVisible} style={{ width: '50vw' }} onHide={() => setIsViewFormVisible(false)}>
                    <div>
                        <p><strong>PC ID:</strong> {viewData._id}</p>
                        <p><strong>PC Name:</strong> {viewData.pcName}</p>
                        <p><strong>PC Code:</strong> {viewData.pcCode}</p>
            ...
            <Dialog header="Edit PC" visible={isEditFormVisible} style={{ width: '50vw' }} onHide={() => setIsEditFormVisible(false)}>
                <form onSubmit={handleEditFormSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
                        <div className="flex flex-col flex-1">
                            <label htmlFor="pcName" className="mb-1 font-semibold text-sm md:text-lg">PC Name</label>
                            <input
                                type="text"
                                id="pcName"
                                name="pcName"
                        ...
                        <div className="flex flex-col flex-1">
                            <label htmlFor="pcCode" className="mb-1 font-semibold text-sm md:text-lg">PC Code</label>
                            <input
                                type="text"
                                id="pcCode"
                                name="pcCode"
            ...
            <div className="mt-8 overflow-x-auto sm:w-full md:w-[100%] xl:w-full">
                ...
                <div className="max-w-[23rem] p-1 md:max-w-[50rem] sm:max-w-[30rem] lg:max-w-[76rem] xl:max-w-[86rem]">
                    <DataTable value={filteredPCs}
                        ...
                        <Column field="status" header="Status" className="py-2 px-4 border-b" />
                        <Column field="pcName" header="PC Name" className="py-2 px-4 border-b" />
                        <Column field="pcCode" header="PC Code" className="py-2 px-4 border-b" />
                        ...
                        <Column field="nos.nosCode" header="NOS Code" className="py-2 px-4 border-b" />
