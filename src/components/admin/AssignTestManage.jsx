...
import {
  assignTestToAssessor,
  setSelectedAssessor,
  setSelectedBatch,
  setSelectedCity,
  setSelectedCountry,
  setSelectedJobRole,
  setSelectedPracticalQuestionSet,
  setSelectedSector,
  setSelectedState,
  setSelectedTcs,
  setSelectedTheoryQuestionSet,
  setSelectedVivaQuestionSet,
} from "../features/assignTestSlice";
import {
  fetchAssessorsByJobRole,
...
const ManageAssignTest = () => {
  ...
  const [isCandidatePhotosRequired, setIsCandidatePhotosRequired] =
    useState(true);
  const [isCandidateVideoRequired, setIsCandidateVideoRequired] =
    useState(true);
  const [isCandidateLocationRequired, setIsCandidateLocationRequired] =
    useState(true);
  const [isCandidateAdharRequired, setisCandidateAdharRequired] =
    useState(true);
  const [isCandidateSelfieRequired, setIsCandidateSelfieRequired] =
    useState(true);
  ...
  const [isRandomizeQuestionsRequired, setIsRandomizeQuestionsRequired] =
    useState(true);
  ...
  const {
    ...
    selectedJobRole,
    selectedSector,
    selectedTheoryQuestionSet,
    selectedPracticalQuestionSet,
    selectedVivaQuestionSet,
    selectedCountry,
    selectedState,
    selectedCity,
    countries,
  ...
  const saveFormData = () => {
    const formData = {
      ...
      isCandidateAdharRequired,
      isCandidateSelfieRequired,
      isAiRequired,
    };
  ...
  const loadFormData = () => {
    ...
    if (formData) {
      ...
      dispatch(
        setSelectedTheoryQuestionSet(formData.selectedTheoryQuestionSet)
      );
      dispatch(
        setSelectedPracticalQuestionSet(formData.selectedPracticalQuestionSet)
      );
  ...
  useEffect(() => {
    ...
    if (selectedTheoryQuestionSet)
      params.set("theoryQuestionSet", selectedTheoryQuestionSet._id);
    if (selectedPracticalQuestionSet)
      params.set("practicalQuestionSet", selectedPracticalQuestionSet._id);
    if (selectedVivaQuestionSet)
      params.set("vivaQuestionSet", selectedVivaQuestionSet._id);
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      selectedPracticalQuestionSet &&
      !selectedPracticalQuestionSet.isTranslated
    ) {
    ...
    const payload = {
      ...
      tc: selectedTc._id,
      ...(selectedTheoryQuestionSet && {
        theoryQuestionBank: selectedTheoryQuestionSet._id,
      }),
      ...(selectedPracticalQuestionSet && {
        practicalQuestionBank: selectedPracticalQuestionSet._id,
      }),
      ...(selectedVivaQuestionSet && {
        vivaQuestionBank: selectedVivaQuestionSet._id,
      }),
    ...
    await dispatch(
      assignTestToAssessor({ batchId: selectedBatch._id, payload })
    ).unwrap();
  ...
  const handleCountryChange = (selectedOption) => {
    const country = selectedOption
      ? countries.find((c) => c._id === selectedOption.value)
      : null;
  ...
  const handleStateChange = (selectedOption) => {
    const state = selectedOption
      ? states.find((s) => s._id === selectedOption.value)
      : null;
  ...
  const handleCityChange = (selectedOption) => {
    const city = selectedOption
      ? cities.find((c) => c._id === selectedOption.value)
      : null;
  ...
  const handleQuestionSetChange = (selectedOption, type) => {
    const questionSet = selectedOption
      ? questionSets.find((c) => c._id === selectedOption.value)
      : null;
    switch (type) {
      case "theory":
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div className="flex flex-col">
            <label
              htmlFor="country"
              className="mb-1 font-semibold text-base md:text-md sm:text-sm lg:text-md ml-1"
            >
              Select Country
            </label>
            <Select
              ...
              value={
                selectedCountry
                  ? {
                      ...
                      label: selectedCountry.name.toUpperCase(),
                    }
                  : null
              }
          ...
          <div className="flex flex-col">
            <label
              htmlFor="state"
              className="mb-1 font-semibold text-base sm:text-sm md:text-md lg:text-md ml-1"
            >
              Select State
            </label>
            <Select
              ...
              value={
                selectedCity
                  ? {
                      ...
                      label: selectedCity.name.toUpperCase(),
                    }
                  : null
              }
          ...
          <div className="flex flex-col flex-1">
            <QuestionSetSelector
              ...
              onChange={(selectedOption) =>
                handleQuestionSetChange(selectedOption, "theory")
              }
            />
          </div>
          <div className="flex flex-col flex-1">
            <QuestionSetSelector
              ...
              onChange={(selectedOption) =>
                handleQuestionSetChange(selectedOption, "practical")
              }
            />
          </div>
          <div className="flex flex-col flex-1">
            <QuestionSetSelector
              ...
              onChange={(selectedOption) =>
                handleQuestionSetChange(selectedOption, "viva")
              }
            />
          </div>
          ...
          <div className="flex flex-col flex-1">
            ...
            <div className="flex items-center">
              <Dropdown
                ...
                options={assessors.map((c) => ({
                  label: `${c.name}- ${c.sipUserId}`,
                  value: c._id,
                }))}
          ...
          <div className="flex items-center space-x-4 mt-5">
            <label className="flex items-center relative w-max cursor-pointer select-none">
              <span className="mb-1 font-semibold text-base sm:text-sm md:text-md lg:text-md ml-1 mr-6">
                Randomize Questions
              </span>
              ...
              <input
                ...
                onChange={(e) =>
                  setIsRandomizeQuestionsRequired(e.target.checked)
                }
              />

              <div
                className={`w-14 h-7 rounded-md relative transition-colors ${
                  isRandomizeQuestionsRequired ? "bg-green-500" : "bg-red-500"
                }`}
              >
                <div
                  className={`w-7 h-7 rounded-md bg-gray-200 absolute top-0 transform transition-transform ${
                    isRandomizeQuestionsRequired ? "translate-x-7" : ""
                  }`}
                />
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div className="flex items-center space-x-4">
            <label className="flex items-center relative w-max cursor-pointer select-none">
              ...
              <div
                className={`w-14 h-7 rounded-md relative transition-colors ${
                  isAiRequired ? "bg-green-500" : "bg-red-500"
                }`}
              >
                <div
                  className={`w-7 h-7 rounded-md bg-gray-200 absolute top-0 transform transition-transform ${
                    isAiRequired ? "translate-x-7" : ""
                  }`}
                />
              </div>
              <span className="absolute font-medium text-xs uppercase right-1 text-white">
          ...
          <div className="flex items-center space-x-4">
            <label className="flex items-center relative w-max cursor-pointer select-none">
              ...
              <div
                className={`w-14 h-7 rounded-md relative transition-colors ${
                  isCandidateSelfieRequired ? "bg-green-500" : "bg-red-500"
                }`}
              >
                <div
                  className={`w-7 h-7 rounded-md bg-gray-200 absolute top-0 transform transition-transform ${
                    isCandidateSelfieRequired ? "translate-x-7" : ""
                  }`}
                />
              </div>
              <span className="absolute font-medium text-xs uppercase right-1 text-white">
          ...
          <div className="flex items-center space-x-4">
            <label className="flex items-center relative w-max cursor-pointer select-none">
              ...
              <div
                className={`w-14 h-7 rounded-md relative transition-colors ${
                  isCandidateAdharRequired ? "bg-green-500" : "bg-red-500"
                }`}
              >
                <div
                  className={`w-7 h-7 rounded-md bg-gray-200 absolute top-0 transform transition-transform ${
                    isCandidateAdharRequired ? "translate-x-7" : ""
                  }`}
                />
              </div>
              <span className="absolute font-medium text-xs uppercase right-1 text-white">
          ...
          <div className="flex items-center space-x-4">
            <label className="flex items-center relative w-max cursor-pointer select-none">
              ...
              <input
                ...
                onChange={(e) =>
                  setIsCandidateLocationRequired(e.target.checked)
                }
              />
              <div
                className={`w-14 h-7 rounded-md relative transition-colors ${
                  isCandidateLocationRequired ? "bg-green-500" : "bg-red-500"
                }`}
              >
                <div
                  className={`w-7 h-7 rounded-md bg-gray-200 absolute top-0 transform transition-transform ${
                    isCandidateLocationRequired ? "translate-x-7" : ""
                  }`}
                />
              </div>
              <span className="absolute font-medium text-xs uppercase right-1 text-white">
          ...
          <div className="flex items-center space-x-4">
            <label className="flex items-center relative w-max cursor-pointer select-none">
              <span className="mb-1 font-semibold text-base sm:text-sm md:text-md lg:text-md ml-1 mr-1">
                Candidate Random Photos
              </span>
              <input
                type="checkbox"
                className="hidden"
              ...
              <div
                className={`w-14 h-7 rounded-md relative transition-colors ${
                  isCandidatePhotosRequired ? "bg-green-500" : "bg-red-500"
                }`}
              >
                <div
                  className={`w-7 h-7 rounded-md bg-gray-200 absolute top-0 transform transition-transform ${
                    isCandidatePhotosRequired ? "translate-x-7" : ""
                  }`}
                />
              </div>
              <span className="absolute font-medium text-xs uppercase right-1 text-white">
          ...
          <div className="flex items-center space-x-4">
            <label className="flex items-center relative w-max cursor-pointer select-none">
              ...
              <div
                className={`w-14 h-7 rounded-md relative transition-colors ${
                  isCandidateVideoRequired ? "bg-green-500" : "bg-red-500"
                }`}
              >
                <div
                  className={`w-7 h-7 rounded-md bg-gray-200 absolute top-0 transform transition-transform ${
                    isCandidateVideoRequired ? "translate-x-7" : ""
                  }`}
                />
              </div>
              <span className="absolute font-medium text-xs uppercase right-1 text-white">
        ...
        <div className="flex space-x-4">
          ...
          <button
            type="button"
            onClick={handleClear}
            className="w-32 py-2 text-white bg-gray-600 hover:bg-gray-700 focus:ring-blue-300 shadow-lg shadow-gray-500/50 font-medium rounded-md text-sm px-5 text-center transition-all duration-300 ease-in-out transform hover:-translate-y-1 hover:scale-110 flex justify-center items-center"
          >
            <span className="pr-2">
