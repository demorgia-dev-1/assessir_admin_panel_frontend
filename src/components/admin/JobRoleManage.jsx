...
import {
  createJobRole,
  deleteJobRole,
  fetchJobRolesBySector,
  lockJobRole,
  setSelectedSector,
  unlockJobRole,
  updateJobRole,
} from "../features/jobRoleSlice";
...
const JobRoleManage = () => {
  ...
  const { sectors, selectedSector, jobRoles } = useSelector(
    (state) => state.jobRole
  );
  ...
  const handleEditFormSubmit = async (e) => {
    ...
    try {
      const resultAction = await dispatch(
        updateJobRole({ _id, updatedJobRole })
      );
  ...
  const filteredJobRoles = selectedSector
    ? jobRoles?.filter((jobRole) => jobRole?.sector?._id === selectedSector._id)
    : [];
  ...
  return (
    <div
      className="max-w-[20rem]  xs:max-w-[23rem] sm:max-w-[60rem] my-2 mt-14 md:max-w-[86rem]  lg:max-w-[100%] xl:w-[100%]
    ...
    >
      ...
      <div className="flex flex-col  space-y-4 my-5">
        <div className="grid grid-cols-1 md:grid-cols-3 p-2 gap-4 bg-gray-100 rounded-lg py-2 pb-6">
          <div className="flex flex-col">
            <label htmlFor="sector" className="mb-1 font-semibold text-lg ml-1">
              Select Sector
            </label>
            <select
              id="sector"
      ...
      {formVisible && (
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="flex flex-col">
              <label htmlFor="nsqf" className="mb-1 font-semibold text-lg ml-1">
                NSQF Level
              </label>
              <input
                type="number"
      ...
      {viewData && (
        ...
        >
          <div>
            <p>
              <strong>Job Role ID:</strong> {viewData._id}
            </p>
            <p>
              <strong>Job Role Name:</strong> {viewData.name}
            </p>
            <p>
              <strong>QP Code:</strong> {viewData.qpCode}
            </p>
            <p>
              <strong>Version:</strong> {viewData.version}
            </p>
            <p>
              <strong>NSQF Level:</strong> {viewData.nsqfLevel}
            </p>
            <p>
              <strong>Total Marks:</strong> {viewData.totalMarks}
            </p>
            <p>
              <strong>Passing Percentage:</strong> {viewData.passingPercentage}
            </p>
            <p>
            ...
            <p>
              <strong>Locked:</strong> {viewData.isLocked ? "Yes" : "No"}
            </p>
          </div>
        </Dialog>
      )}
      <Dialog
        header="Edit Job Role"
        ...
        <div className=" p-1 md:max-w-full lg:max-w-[100%] max-w-[20rem]  xs:max-w-[23rem] sm:max-w-[50rem]">
          ...
          >
            <Column
              ...
              header="Actions"
              className="sm:px-1 py-2 px-4 border-b"
            />
            <Column
              field="_id"
              header="Id"
            ...
            <Column
              field="totalMarks"
              header="Total Marks"
              className="sm:px-1 py-1 px-1 border-b"
            />
          </DataTable>
        </div>
