...
import {
  clearCandidates,
  fetchCandidateByBatchId,
} from "../features/candidateBatchStatusSlice";
...
const CandidatesForBatch = () => {
  ...
  const { candidates, totalCandidates } = useSelector(
    (state) => state.candidateEvidence
  );
  const {
    batches,
    jobRoles,
  ...
  const handleResetTheory = async () => {
    ...
    const candidateIdsToReset = Object.keys(resetStatus).filter(
      (candidateId) => resetStatus[candidateId].checked
    );
    ...
    try {
      const response = await axios.post(
        `${BASE_URL}company/batches/${selectedBatch._id}/reset-theory-test`,
        {
  ...
  const handleResetPractical = async () => {
    ...
    try {
      const response = await axios.post(
        `${BASE_URL}company/batches/${selectedBatch._id}/reset-practical-test`,
        {
          candidates: candidateIdsToReset,
        },
        {
  ...
  const handleResetViva = async () => {
    ...
    try {
      const response = await axios.post(
        `${BASE_URL}company/batches/${selectedBatch._id}/reset-viva-test`,
        {
          candidates: candidateIdsToReset,
        },
        {
  ...
  const handleDownloadLogs = async (candidateId) => {
    try {
      ...
      const response = await axios.get(
        `${BASE_URL}company/candidates/${candidateId}/theory-logs`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleDownloadSelectedExcel = () => {
    ...
    const tableRows = selectedCandidates.map((candidate) => [
      ...
      candidate.practicalExamStatus === "submitted"
        ...
        : "NotSubmitted",
      candidate.vivaExamStatus === "submitted" ? "Submitted" : "NotSubmitted",
      candidate.theoryStartedAt
        ? convertToIST(candidate.theoryStartedAt)
        : "Invalid Date",
      candidate.theorySubmittedAt
        ? convertToIST(candidate.theorySubmittedAt)
        : "Invalid Date",
    ]);
  ...
  return (
    <div
      className="w-auto max-w-[20rem]  xs:max-w-[23rem] sm:max-w-[60rem] my-2  md:max-w-[86rem]  lg:max-w-[100%] xl:w-[100%]
    ...
    >
      ...
      <div className="flex flex-col space-y-4 my-5">
        <div className="grid p-2 grid-cols-1 md:grid-cols-3 gap-4 bg-gray-100 rounded-lg py-2 pb-6">
          ...
          <div className="flex flex-col">
            <label htmlFor="batch" className="mb-1 font-semibold text-md ml-1">
              Select Batch
            </label>
            <Dropdown
              id="batch"
          ...
          <div className="max-w-[23rem] p-1 md:max-w-[50rem] sm:max-w-[30rem] lg:max-w-[76rem]">
            ...
            >
              <Column
                ...
                body={(rowData) => (
                  <input
                    ...
                    onChange={(e) =>
                      handleResetChange(rowData._id, e.target.checked)
                    }
                  />
                )}
              ...
              <Column
                ...
                className="py-2 px-4 bg-sky-50 border-b header-gap text-gray-700 text-sm font-semibold"
              />
              <Column
                field="name"
                header="NAME"
                className="py-2 px-4 border-b bg-sky-50 header-gap text-gray-700 font-semibold text-sm"
              />
              <Column
                field="isPresentInTheory"
                header="THEORY PRESENT"
                className="py-2 bg-sky-50 px-4 border-b header-gap"
                body={(rowData) =>
                  presentTemplate(rowData, "isPresentInTheory")
                }
              />
              <Column
                field="isPresentInPractical"
                header="PRACTICAL PRESENT"
                className="py-2 px-4 bg-sky-50 border-b header-gap"
                body={(rowData) =>
                  presentTemplate(rowData, "isPresentInPractical")
                }
              />
              <Column
                field="isPresentInViva"
                header="VIVA PRESENT"
                className="py-2 bg-sky-50 px-4 border-b header-gap"
                body={(rowData) => presentTemplate(rowData, "isPresentInViva")}
              />
              <Column
                field="isEvidanceUploaded"
                header="EVIDENCE UPLOADED"
                className="py-2 bg-sky-50 px-4 border-b header-gap"
                body={(rowData) => yesNoTemplate(rowData, "isEvidanceUploaded")}
              />
              <Column
                field="theoryExamStatus"
                header="THEORY STATUS"
                className="py-2 bg-sky-50 px-4 border-b header-gap"
                body={(rowData) =>
                  examStatusTemplate(rowData, "theoryExamStatus")
                }
              />
              <Column
                field="theoryStartedAt"
                header="THEORY START TIME"
                className="py-2 bg-sky-50 px-4 border-b header-gap text-sm text-gray-700 font-semibold"
                body={(rowData) =>
                  rowData.theoryStartedAt
                    ? convertToIST(rowData.theoryStartedAt)
                    : "Invalid Date"
                }
              />
              <Column
                field="theorySubmittedAt"
                header="THEORY SUBMIT TIME"
                className="py-2 bg-sky-50 px-4 border-b header-gap text-sm text-gray-700 font-semibold"
                body={(rowData) =>
                  rowData.theorySubmittedAt
                    ? convertToIST(rowData.theorySubmittedAt)
                    : "Invalid Date"
                }
              />
              <Column
                field="practicalExamStatus"
                header="PRACTICAL STATUS"
                className="py-2 bg-sky-50 px-4 border-b header-gap"
                body={(rowData) =>
                  examStatusTemplate(rowData, "practicalExamStatus")
                }
              />
              <Column
                field="vivaExamStatus"
                header="VIVA STATUS"
                className="py-2 bg-sky-50 px-4 border-b header-gap"
                body={(rowData) =>
                  examStatusTemplate(rowData, "vivaExamStatus")
                }
              />
              <Column
                body={(rowData) =>
                  reasonTheorySubmission(rowData, "reasonOfTheorySubmission")
                }
                header="THEORY SUBMISSION REASON"
                className="py-2 bg-sky-50 px-4 border-b header-gap text-sm text-gray-700 font-semibold"
              />
              ...
              <Column
                header="EVIDENCE"
                body={viewEvidenceTemplate}
                className="py-2 bg-sky-50 px-4 border-b text-gray-700 font-semibold"
              />
            </DataTable>
