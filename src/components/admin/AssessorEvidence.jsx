...
const AssessorEvidence = () => {
  ...
  const handleAddQuestion = async () => {
    ...
    try {
      const response = await axios.post(
        `${BASE_URL}company/assessors/ratings/questions`,
        newQuestionObj,
        {
          headers: {
      ...
      if (response.status === 200) {
        setQuestions([
          ...questions,
          { id: questions.length + 1, text: newQuestion.trim() },
        ]);
        setNewQuestion("");
  ...
  const handleSubmitRatings = async () => {
    ...
    try {
      const response = await axios.post(
        `${BASE_URL}company/assessors/ratings/questions/bulk-ratings`,
        payload,
        {
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <div className="w-full mx-auto mt-14 sm:mt-5 p-0 sm:p-2 py-4 bg-white border-2 border-white/10 items-center overflow-hidden duration-200 ease-in-out text-black rounded-lg shadow-lg flex-grow-0">
      <div className="flex flex-col space-y-4 my-3">
        <span className="text-xl font-bold mb-2 ml-1 p-2 text-center sm:text-left">
          Assessor Evidence
        </span>
            ...
            <div className="p-4 bg-gray-100 rounded-lg shadow-md flex justify-between items-center">
              <div>
                <p>
                  ...
                  {selectedBatch.assessor?.name || "N/A"}
                </p>
                <p>
                  <strong>Assessor Email: </strong>{" "}
                  {selectedBatch.assessor?.email || "N/A"}
                </p>
                <p>
                  <strong>Assessor Phone: </strong>{" "}
                  {selectedBatch.assessor?.phone || "N/A"}
                </p>
                <p>
                  <strong>SIP User Id: </strong>{" "}
                  {selectedBatch.assessor?.sipUserId || "N/A"}
                </p>
                <p>
                  <strong>Assessor Reached Location: </strong>
                  {location}
                </p>
                <p>
                  <strong>Assessor Reached Time:</strong>{" "}
                  {selectedBatch.assessorReachedAt
                    ? new Date(selectedBatch.assessorReachedAt).toLocaleString()
                    : "N/A"}
                </p>
                <div className="flex items-center mb-2">
                  <p className="mr-2">
                    <strong>Assessor Group Photo:</strong>{" "}
                    {selectedBatch.assessorGroupPhoto ? (
                      <a
                        href={selectedBatch.assessorGroupPhoto}
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        <FiDownload className="inline-block h-5 w-5 ml-4 text-center text-green-700" />
                      </a>
                    ) : (
                      "N/A"
                    )}
              ...
              <div>
                {selectedBatch.assessorGroupPhoto && (
                  <img
                    src={selectedBatch.assessorGroupPhoto}
                    alt="Assessor Group"
                    className="mt-2 rounded-lg shadow-md cursor-pointer"
                    onClick={() =>
                      handleImageClick(selectedBatch.assessorGroupPhoto)
                    }
                  />
                )}
              </div>
        ...
        <div className="mt-5 space-x-4 hidden">
          ...
          <Button
            label="Add"
            icon="pi pi-plus"
            className="bg-purple-600 hover:bg-purple-700 text-white font-semibold py-1 sm:py-2 sm:px-4 rounded-md text-xs sm:text-sm w-full sm:w-auto ml-2 lg:text-xs"
        ...
        {selectedBatch && (
          <div className="mt-5">
            <Button
              label="Rate Assessor"
              className="bg-orange-600 hover:bg-orange-700 text-white font-semibold py-1 sm:py-2 sm:px-4 rounded-md text-xs sm:text-sm w-full sm:w-auto ml-2 lg:text-xs"
              icon="pi pi-star"
        ...
        <Dialog
          ...
          style={{ width: "50vw" }}
          header={
            <h2 className="text-lg font-bold text-gray-800">Rate Assessor</h2>
          }
          ...
          {questions?.length > 0 ? (
            <div className="space-y-3">
              {questions.map((question) => (
                ...
                >
                  <p className="font-medium text-gray-800 text-sm sm:text-base lg:text-md">
                    {question.text}
                  </p>
                  <Rating
      ...
      <Dialog
        ...
        style={{ width: "30vw" }}
        header={`${selectedBatch?.assessor?.name}` || ""}
        onHide={() => setDialogVisible(false)}
      >
        {zoomImage && (
          <img
            src={zoomImage}
            alt="Zoomed Assessor Group"
